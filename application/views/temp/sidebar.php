...
if ($role_id == 1) { //admin
  ...
  $foto = $user->image;
  $nama = $user->name;
} else if ($role_id == 2) { //wamur
  $this->db->where('nis', $this->session->userdata('email'));
  $user = $this->db->get('88_siswa')->row();
  $foto = $user->image;
  $nama = $user->name;
} else if ($role_id == 3) { //walas
  $this->db->where('id_guru', $this->session->userdata('email'));
  $user = $this->db->get('88_guru')->row();
  $foto = $user->image;
  $nama = $user->name;
} else { //gulas
  // $this->db->where('email', $this->session->userdata('email'));
...
<body class="hold-transition sidebar-mini layout-fixed">
  <div class="wrapper">
    ...
    <!-- Navbar -->
    <nav class="main-header navbar navbar-expand navbar-success navbar-dark ignielPelangi">
      <!-- Left navbar links -->
      <ul class="navbar-nav">
        <li class="nav-item">
    ...
    <aside class="main-sidebar sidebar-dark-primary elevation-4">
      ...
      <div class="sidebar">
        ...
        <div class="user-panel mt-3 pb-3 mb-3 d-flex">
          <div class="image">
            <img src="<?= base_url('assets/foto_user/') . $foto ?>" class="img-circle elevation-2" alt="User Image">
          </div>
          <div class="info">
          ...
          <?php foreach ($menu as $m) : ?>
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
              <?php if ($this->session->userdata('role_id') == 1) : ?>
                <li class="nav-header text-secondary"><?= $m['menu']; ?>
                <?php endif ?>
